...
- Required variables (only these are recognized by the codebase):
  - REACT_APP_BEAR_API_URL: Backend base URL without a trailing slash (e.g., http://localhost:3001).
  - REACT_APP_REFRESH_INTERVAL_SECONDS: Auto-refresh interval in seconds (default 10).
- Do NOT use duplicated keys like REACT_APP_REACT_APP_BEAR_API_URL or REACT_APP_REACT_APP_REFRESH_INTERVAL_SECONDS — they are ignored.
...
- Verify the backend is reachable at ${REACT_APP_BEAR_API_URL}/api/bears in a browser tab.
- Ensure the URL scheme and host match (no HTTP/HTTPS mismatch).
- Confirm CORS: the backend enables CORS for /api/* and allows all origins by default.
